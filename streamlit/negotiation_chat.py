...
def main(): 
    ...
    #selection = st.sidebar.radio("Go to", ["Home", "Questionnaire", "Negotiation 1", "Negotiation 2"])
        ...
        st.write("""
             Please engage in a negotiation with this chatbot, powered by GPT-3.5 Turbo. Select the chatbot's personality and your preferred scenario. Start the negotiation by entering the following message below into the message prompt **'Let's discuss our perspectives on how we believe the distribution should be managed.'** After the chatbot responds continue the negotiation adhering to the role described in the selected scenario. After completing this session (the negotiation is limited to 7 rounds, it will be interrupted if reached them), proceed to the next page for a continuation with a GPT-4 Turbo chatbot.
        """)
